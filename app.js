...
app.get('/divelogs', function(req, res) {
    let queryDivelogs = "SELECT divelog_id, Divelogs.dive_id, DATE_FORMAT(date, '%d-%M-%Y') as date, max_depth, duration, Divers.diver_id, diver_name FROM Divelogs LEFT JOIN Divers ON Divelogs.diver_id = Divers.diver_id JOIN Dives ON Divelogs.dive_id = Dives.dive_id ORDER BY divelog_id;";
                ...
                if (error) {
                    ...
                    db.pool.query(queryDivers, function(error, diverData, fields) {
                        if (error) {
                            ...
                            for (let i=0; i < rows.length; i++) {
                                // Capture NULL values
                                    let diver_name = rows[i].diver_name;
                                    if (diver_name === null)
                                    {
                                        rows[i].diver_name = "NULL"
                                    }
                                }
